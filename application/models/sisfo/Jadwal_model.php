...
class Jadwal_model extends CI_Model
{
	...
	public function tambahDataJadwal()
	{
		...
		$i = 1;
		while ($i <= $num) {
			$jam = $this->input->post('jam_mulai'.$i, true).' - '.$this->input->post('jam_akhir'.$i, true);

			$data = [
				'hari' => $this->input->post('hari', true),
				'jam' => $this->input->post('jam', true),
				'jam' => $jam,
				'id_mengajar' => $this->input->post('id_mengajar'.$i, true),
				'id_ruangan' => $this->input->post('id_ruangan'.$i, true),
